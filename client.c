...
int get_content_name(const unsigned char *interest_msg,  struct ccn_parsed_interest *pi, char **interest_name,  char **new_interest_name)
{
...
int main (int argc, char **argv)
{
   ...
   /* int timeout_ms;
    res = sscanf(argv[2], "%d", &timeout_ms);
    if(res == 0)
    {
    ...
    //get the node identifier, IP address for now
    
    res = get_ip_addresses();

    if(res !=0)
    {
        fprintf(stderr, "Can not get identifier");
        exit(1);
    }*/
